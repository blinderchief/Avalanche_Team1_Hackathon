...
export function MarketCard({ market, className = "" }: MarketCardProps) {
  ...
  const getCategoryColor = (category: string) => {
    switch (category.toLowerCase()) {
      ...
      case 'stocks': return 'bg-green-500/10 text-green-500 border-green-500/20';
      case 'space': return 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20';
      case 'economics': return 'bg-amber-500/10 text-amber-500 border-amber-500/20';
      case 'tech': return 'bg-pink-500/10 text-pink-500 border-pink-500/20';
  ...
  const isTrending = market.participants > 500 || parseFloat(market.volume.replace(/[^0-9.]/g, '')) > 1000;
    ...
    <Link to={`/markets/${market.id}`} className={`block ${className}`}>
      <Card className="market-card border-border/50 hover:border-primary/50 overflow-hidden transition-all duration-300 h-full relative">
        ...
        <CardContent className="pt-0 pb-4 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-success/10 border border-success/20 rounded-lg p-3 relative overflow-hidden">
              <div className="absolute bottom-0 left-0 h-1 bg-success" style={{ width: `${market.yesPrice * 100}%` }}></div>
              <div className="text-xs text-success font-medium mb-1 flex items-center">
                YES
                {market.yesPrice > 0.6 && <ArrowTrendingUpIcon className="w-3 h-3 ml-1" />}
              </div>
            ...
            <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 relative overflow-hidden">
              <div className="absolute bottom-0 left-0 h-1 bg-destructive" style={{ width: `${market.noPrice * 100}%` }}></div>
              <div className="text-xs text-destructive font-medium mb-1 flex items-center">
                NO
                {market.noPrice > 0.6 && <ArrowTrendingUpIcon className="w-3 h-3 ml-1" />}
              </div>
        ...
        <CardFooter className="pt-0 pb-4 px-4 relative z-10">
          <Button variant="ghost" className="w-full text-sm h-9 hover:bg-primary/10 hover:text-primary">
            Trade Now
          </Button>
        </CardFooter>
      </Card>
